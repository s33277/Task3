...
public sealed class LinqExercises
{
    ...
    public IEnumerable<string> Task04_FirstAnalyticsCourse()
    {
        ...
        [
            UniversityData.Courses
                ...
                .FirstOrDefault() ?? "No course named Analytics."
        ];
    }
    ...
    {
        return UniversityData.Students
            .Select(s => s.City).Distinct().OrderBy(c => c);
    }
    ...
    public IEnumerable<string> Task14_AverageGradePerCourse()
    {
        return UniversityData.Enrollments
            ...
            .GroupBy(ec=>ec.c.Title)
            .Select(g => $"{g.Key}, {g.Average(ec=>ec.e.FinalGrade)}");
    }
    ...
    public IEnumerable<string> Task15_LecturersAndCourseCounts()
    {
        return UniversityData.Lecturers
            .GroupJoin(UniversityData.Courses, l => l.Id, c => c.LecturerId,
                (l, c) => new { FullName = $"{l.FirstName} {l.LastName}", Count = c.Count() })
            .Select(fc => $"{fc.FullName}, {fc.Count}");
    }
    ...
    public IEnumerable<string> Task16_HighestGradePerStudent()
    {
        return UniversityData.Students
            .Join(UniversityData.Enrollments, s=>s.Id, e=>e.StudentId,
                (s, e)=> new {s, e})
            .Where(se=> se.e.FinalGrade != null)
            .GroupBy(se=> new { se.s.FirstName, se.s.LastName })
            .Select(g => $"{g.Key.FirstName} {g.Key.LastName}, {g.Max(se=>se.e.FinalGrade)}");
    }
    ...
    {
         return UniversityData.Courses
             .Join(UniversityData.Enrollments, c=> c.Id, e => e.CourseId,
                 (c, e) => new {c, e})
             .Where(ce=>ce.c.StartDate is { Month: 4, Year: 2026 })
             .GroupBy(ce=>ce.c.Title)
             .Where(g=>g.All(ce=>ce.e.FinalGrade==null))
             .Select(g => g.Key);
    }
    ...
    {
        return UniversityData.Lecturers
            .SelectMany(l=> UniversityData.Courses.Where(c=>c.LecturerId==l.Id).DefaultIfEmpty(),
                (l,c)=> new {l, c})
            .SelectMany(lc=>UniversityData.Enrollments.Where(e=>e.CourseId==(lc.c!=null?lc.c.Id:-1)).DefaultIfEmpty(),
                (lc, e)=> new {lc, e})
            .Where(lcE=> lcE.e is { FinalGrade: not null })
            .GroupBy(lcE=> new {lcE.lc.l.FirstName, lcE.lc.l.LastName})
            .Select(g => $"{g.Key.FirstName} {g.Key.LastName}, {g.Average(lcE => lcE.e.FinalGrade)}");
    }
